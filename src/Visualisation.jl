...
function _make_pvd_distributed(dir_loc,simName,convert2seconds; threedims=false)
  ...
  files = filter(x->endswith(x, extension), readdir((dir_loc)))
    ...
    for f in files
      ...
      time = str[length(simName)+2:length(str)-string_length]
      println(time)
      t = parse(Float64,time)/(convert2seconds)
      t2 = string(t)
      # println(t)
